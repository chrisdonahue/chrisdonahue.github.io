...
window.jeffDonahue = window.jeffDonahue || {};

(function () {
	function scrambledString(tag, initScrambledString, initScrambledStringIndices) {
		this.tag = tag;
		...
		this.rescramble();
		this.tag.innerHTML = '(<a class="email-unscrambler" style="cursor: pointer;">click to unscramble</a>) ' + this.string;
		var that = this;
		this.tag.onclick = function () {
			that.initAnimatedBubbleSort();
		};
	}
	...
	function initAnimatedBubbleSort() {
		var that = this;
		this.interval = setInterval(function () { that.bubbleSortStep(); }, 4);
	}
	...
	window.jeffDonahue.scrambledString = scrambledString;
})();