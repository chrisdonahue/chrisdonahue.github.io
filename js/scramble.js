...
function scrambledString(tag, objName, initScrambledString, initScrambledStringIndices) {
	...
	this.rescramble();
	this.tag.innerHTML = this.string + ' <a id="unscramble" onClick="' + this.objName + '.initAnimatedBubbleSort();">(unscramble)</a>';
}
...
function initAnimatedBubbleSort() {
	this.interval = setInterval(this.objName + '.bubbleSortStep()', 2);
}
